...
class DocumentationRenderer(var element: PsiElement, private var original: PsiElement?) {
    ...
    fun onHover(): String {
        when {
            DejavuLexer.Keywords.contains(element.elementType) -> return RequestDocument.keyword(element.text).send()
            DejavuLexer.Operators.contains(element.elementType) -> return RequestDocument.operator(element.text).send()
            else -> {}
        }
        when (element) {
