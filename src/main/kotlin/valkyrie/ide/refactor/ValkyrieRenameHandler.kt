...
class ValkyrieRenameHandler : RenameHandler {
    override fun invoke(project: Project, editor: Editor?, file: PsiFile?, dataContext: DataContext?) {
        if (file !is DejavuFileNode || editor == null) {
            return
        }
        val offset = editor.caretModel.offset
        editor.scrollingModel.scrollToCaret(ScrollType.MAKE_VISIBLE)
        val element = file.findElementAt(offset) ?: return
        if (DejavuLexer.Keywords.contains(element.elementType)) {
            val message = dejavu.language.DejavuBundle.message("rename.invalid.keyword", element.elementType!!)
            CommonRefactoringUtil.showErrorHint(project, editor, message, "getTitle", "getHelpId")
        } else if (DejavuLexer.Identifiers.contains(element.elementType)) {
